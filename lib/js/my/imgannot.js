/// imgannot.js by KANZAKI, Masahide. ver 3.15, 2018-06-30. MIT license.
"use strict";
var at, Mav = {}, Mau, Mwa;
...
var Mia = {
	...
	prep_osd: function (tile, type){
		...
		this.tindex.prepare(this.osdv, (tile instanceof Array));
...
Mpj.webannot = {
	...
	body_choice: function(bodytype, items){
		var bodyvalue;
		if(bodytype === "Choice") bodyvalue = Mut.str.lang_obj(items);
		else{
...
var Manno = {
	...
	flush: function(newuri){
		...
		function flush_annotation(uri, that){
			...
			var svgstate = Array(that.num_anbox);	//各注釈がSVGノードを持つかどうかを記録する配列。Annotationからさらに"application/ld+json"として入れ子呼び出ししている場合、処理済みの注釈ボックス分をundefinedで埋めておく
...
Manno.svg = {
	...
	flush_overlay: function(ci){
		if(!this.svg_node) this.svg_node = Mia.osdv.viewer.svgOverlay();
...
var Miiif = {
	...
	proc_one_embed: function(annors, annot, idx, cvinst, context){
		...
		function no_ci_url(chkres, original, tt){
			console.warn("Unknown (no cinfo) target", original, (chkres[1] ? "with " + chkres[1].substr(0,20) :"no frag"));	//tt, tgparts, "for", annors
			...
			OpenSeadragon.makeAjaxRequest({
				...
				"success": function(xhr) {
					...
					if((uri_b = Mut.scheme.exists_another(oa, uri, true))){
						...
						if(!checked) test_complete(uri, i);
					}
...
Tindex.prototype = {
	...
	init_vinfo: function(inipos, from_search){
		...
		if(Miiif.cvs.thumb_count) this.viewinfo.appendChild(this.mia.gallery.prep_btn());
...
Canvas.prototype = {
	...
	proc_one_item: function(item, default_type){
		...
		body_arr.forEach(function(bodyr){
			...
			if(!type){
				if(default_type){
					type = default_type;
					console.warn("No type for item. Assumes default type", type);
				}
			}
			switch(type){
			case "Image" :
			...
			case "Audio" :
			case "Sound" :
				this.set_mediaurl(bodyid, type, trgt, bodyr.format, item.timeMode);
				break;
			case "TextualBody" :
			...
			case this.iiif.v.annocollection :
				...
				break;
			case "Choice" :
				this.set_choice(bodyr, trgt, item.stylesheet);
				break;
			case "PhysicalObject" :
			case "ThreeD" :
			...
			case undefined :
				type = this.guess_type(bodyr, trgt, item.stylesheet);
				break;
			case "Canvas":
				var mfinfo = this.set_mediaurl(bodyid, type, trgt, bodyr.format, item.timeMode),
			...
			if(type) Miiif.last_valid_type = type;
		}, this);
	},
	...
	set_choice: function(bodyr, trgt, styles){
	...
	set_canvas_meta: function(obj){
		var meta = "", mar = [],
		more = {"seealso": "See also", "related": "Related", "rendering": "Rendering", "within": "Within", "partOf": "Part of"};
...
var Muib = {
	...
	meta: {
		...
		generate: function(def, pelt){
			...
			pelt.innerHTML = "<em>Document info <a href=\""+ Mia.opts.v.u + "\">" +
			logo + doctype  + "</a></em>" + lang_selector + ":";
	...
	anno: {
		...
		hilite: function(annot){
			...
			if(this.clip_hilited){
				...
				if(!this.pop_showing){
					mviewer.world.getItemAt(0).setOpacity(1);
...
var Mut = {
	...
	html: {
		...
		fold_text: function (text, type="long"){
			var res,
			limit = this.set_limit(text, type),
			thld = limit - 6,
			notag = text.replace(/<.*?>/g, "");
			if(notag.length > limit){
				res = notag.substr(0, thld) + "<span class=\"more\">" +
	...
	str: {
		...
		set_prop: function(mode){
			this.prop = (mode === "iiif" && Miiif.vers.v < 3) ?
		...
		lang_obj: function(arrobj, glue="; "){
			...
			if(res[this.preflang].length) {
				...
				Object.keys(res).forEach(function(lang){
					if(res[lang].length) ro = res[lang].join(glue);
				});
				if(ro === {}) ro = arrobj[0];
			}
