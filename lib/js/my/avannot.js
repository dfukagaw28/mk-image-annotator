...
Mav.annot = {
	...
	resolve_deferred: function(oa, idx){
		...
		if(Object.keys(Mia.defer.paint[idx]).length) usepaint = test_overlap(Mia.defer.paint[idx], usepaint);
		...
		function test_overlap(painting, defval){
			for(var uri in painting){
				...
				for(var i=0, n=painting[uri].length; i<n; i++){
					...
					if(!loc_count[an.loc]) loc_count[an.loc] = 1;
						...
						console.log("☆use non painting mode for overlapping annotations");
						return false;
					}
				}
			}
	...
	check_now: function(now){
		...
		if(this.end[hidetime] || this.ed_media[hidetime]){
			...
			if(this.end[hidetime]) this.end[hidetime].forEach(function(an){
				...
				else this.test_overlap(now, 1, an);
			}, this);
	...
	test_overlap: function(now, deleted, an){
		console.log(" -->test_overlap", this.test_showing(), deleted);
