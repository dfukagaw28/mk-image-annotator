/// avannot.js by KANZAKI, Masahide. ver 2.4, 2018-03-26. MIT license.
"use strict";
var Mia, Mpj, Manno, Muib, Miiif, Mut, Mwa, anno, at, YT;
var Mav = {
	v: null,
	useOsdvOverlay: true,
	...
	start_pos: function(vurl){
		...
		if(Mia.opts.v.t){
			...
			if(ci.mf && ci.mf[this.vinfo.url][0].bodyt) this.state.initpos = ci.mf[this.vinfo.url][0].bodyt[0];
	...
	setup_annobox: function(){
		...
		Mwa.antrs.addHandler("onMouseOverAnnotation", function(an){
			if(Mavannot.showing[an.id] && Mavannot.showing[an.id] === "clone") Mavannot.clone[an.id].style.opacity = 0;
		});
		Mia.opts.set_style("border_color:yellow;", true);
	},
	map_handlers: function(type){
		Manno.set_changed = function(delta, an){Mavannot.set_changed(delta, an);};
		Manno.get_annotation = function(){return Mavannot.get(type);};
		Mia.key_uri = function(){return Mav.key_uri();};
		Manno.set_new = function(an){Mavannot.set_new(an);};
	},
	...
	setup_page_media: function(url, stepfade, from_subcanvas){
		...
		if(cinfo.mf) for(var murl in cinfo.mf){
			cinfo.mf[murl].forEach(function(o){
				var id = Mavannot.gen_id(murl, o),
				minfo = Mavannot.all_medias[id];
				if(!Mav.melts[id]){
					if(!Mavclock.use && Mav.type){
						Mav.melts[id] = true;
					}else{
						this.gen_elt(id, murl, o, false, from_subcanvas ? url : null);
						if(o.type === "ThreeD") return;
						else if((!o.t || o.t[0] === 0) && o.class !== "highlight") Mav.melts[id].style.display = "block";
					}
				}
				if(o.type !== "Canvas" && minfo.rect){
		...
		if(this.v && !this.state.volume_done){
			Mav.v.volume = Mav.opts.volume;
			Mav.state.volume_done = true;
		}
	},
	...
	set_newpage: function(newuri, prevuri, from_tindex, now, pos){
		...
		Mavannot.update_page_anno(newuri, prevuri);
		if(this.state.audiothubm) update_audio_thumb(this.cinfo);
		Mavclock.setup();
		if(Mavclock.timer) window.clearInterval(Mavclock.timer);
		this.setup_page_media(newuri, stepfade, false);
		this.set_region_info();
		if(now !== undefined && Mavclock.use){
			Mavclock.playing = false;
	...
	setup_elt: function(){
		...
		this.setup_osdcnvs();
		Mia.elt.osd_cntner.insertBefore(this.v, Mia.osdv.viewer.buttons.element.parentNode.parentNode);

		if(Mia.env.is_touch_dev){
			if(Mav.type !== "youtube") Mav.v.load();
			add_playbtn();
		}
	...
	gen_elt: function(id, murl, o, salone_init, subcanvas_uri){
		...
		switch(o.type){
		case "Text":
			elt = gen_new_elt(id, "div", o.val, [["class", "text" + o.class]]);
			if(o.class === "paint") elt.onclick = Manno.toggle_textpaint;
			break;
		case "Video":
		case "Audio":
		case "Sound":
			elt = gen_media_elt(id, murl, o, salone_init, this);
			break;
		case "ThreeD":
			Muib.threed.load(murl, o.format, this.melts[subcanvas_uri]);
			return;
		case "Canvas":
			var mi = Mavannot.all_medias[id];
			if(!mi){//.cvinst
				mi = Mavannot.all_medias[id] = {"type": "Canvas"};
				mi.cvinst = new Subcanvas(murl, id, cinfo, osdv);
			}
			Mia.cinfo[murl].mavinfo = mi;
			mi.cvinst.osd_setup(o);
			Mavannot.setup(murl);
			break;
		}
		if(salone_init) return elt;
		if(o.type !== "Canvas") this.append_and_set(id, o, cinfo, osdv, subcanvas_uri ? true : false);
		
		function gen_media_elt(id, murl, o, salone_init, that){
			var attr = o.choice ? [] : [["src", murl]],
			type = o.type.toLowerCase() === "video" ? "video" : "audio";
			if(Mav.opts.ctrls) attr.push(["controls", ""]);
			if(o.vtt) attr.push(["crossorigin", "anonymous"]);
			var elt = gen_new_elt(id, type, "", attr);
			if(!salone_init && !Mavclock.use){
			...
			if(!Mav.v) Mia.elt.media = Mav.v = elt;//Mav.melts[id];
		}
	...
	append_and_set: function(id, o, cinfo, osdv, from_subcanvas){
		Mav.melts[id].style.display = "none";
		if(o.pos) this.set_eltpos(id, o, cinfo, osdv.viewer, from_subcanvas);
	},

	set_eltpos: function(id, o, cinfo, mviewer, from_subcanvas){
		var pos = o.pos,
		minfo = Mavannot.all_medias[id];
		if(!minfo.orgp) minfo.orgp = o.orgp;
		
		var rect = Muib.tool.pix2viewportRect(o.orgp, cinfo.dim.x);
		minfo.rect = rect;
		if(from_subcanvas){
			mviewer.addOverlay(Mav.melts[id], rect); //|| o.type === "Canvas"
	...
	sec_now: function() {
		if(Mavclock.use) return Mavclock.now;
		else return Math.floor(this.v.currentTime);
	},
	start: function(){
		if(Mavclock.use) Mavclock.start();
		else Mav.v.play();
	},
	pause: function(){
		if(Mavclock.use) Mavclock.pause();
		else Mav.v.pause();
	},
	resume: function(){
		if(Mavclock.use) Mavclock.resume();
		else Mav.v.play();
	},
	is_paused: function(){
		return Mavclock.use ?
...
var Mavannot = {
	...
	setup: function(keyuri){
		...
		if(Mav.cinfo.duration) Mavclock.add_timeline(Mav.cinfo.duration);
	},
	...
	resolve_deferred: function(oa, idx){
		var usepaint = (Miiif.tfmedia || Miiif.locfmedia || Mia.opts.v.tp) ? true : false;
		["paint", "highlight"].forEach(function(mtvn_type){
			if(!Mia.defer[mtvn_type][idx] || Object.keys(Mia.defer[mtvn_type][idx]).length === 0) return;
			else if(usepaint || mtvn_type==="highlight"){	//
				for(var uri in Mia.defer[mtvn_type][idx]) Mia.defer[mtvn_type][idx][uri].forEach(function(tp){
					var mf = {
						"type": "Text",
						"class": mtvn_type,
						"id": this.gen_id(tp.target.source, tp),
						"val": at.gettext(tp)
					};
					Canvas.set_mf(null, mf, tp.loc, tp.trange);
					Mut.obj.prepare(Mia.cinfo[uri], "mf");
					Mut.arr.add(Mia.cinfo[uri].mf, uri, mf);
					Mut.arr.add(Manno.tp, uri, tp);
					if(t==="highlight") Mia.tindex.set_data_an_attr(tp.length, uri);
				}, this);
			}else{
	...
	setup_media: function(from_subcanvas){
		...
		function add_media(url){
			...
			if(ci.layer) ci.layer.forEach(function(o, i){
				if(o.done) return;
				add_images(urepo, o, i);
			});
			if(ci.mf) for(var murl in ci.mf) ci.mf[murl].forEach(function(o){
				if(o.done) return;
				add_avc(urepo, o, i, murl);
			});
		}
		function add_images(urepo, o, i){
			var id = Mavannot.get_tileid(o, ci);
			if(o.trange){
				var
				st = Math.floor(o.trange[0]) + t_offset, 
				ed = Math.floor(o.trange[1]) + t_offset;
				if(st === ed) ed++;
				Mut.arr.add(urepo.st_media, st, {"id": id, "end": ed});
				Mut.arr.add(urepo.ed_media, ed, id);
				add_durs(urepo.durs, st, ed, id, "image", null);
				that.all_medias[id] = {"type": "Image", "canvas": url, "parent": ci.parent_cv, "self": o.tid, "orgp": o.orgp};
			}
			if(i > 0 || id === ci.imgurl) urepo.layers.push(id);
			
			o.done = true;
		}
		function add_avc(urepo, o, i, murl){
			var id = that.gen_id(murl, o), st, ed, cvdur;
			if(o.t){
				st = Math.floor(o.t[0]) + t_offset;
				ed = Math.floor(o.t[1]) + t_offset;
				Mut.arr.add(urepo.ed_media, ed, id);
			}else{
				st = 0;
				ed = ci.duration;
			...
			if(o.bodyt){
				...
				if(bdur !== cvdur) rate = time_mode(id, o, st, bdur, cvdur, bst, urepo);
			}else{
				bst = 0;
			}
			...
			that.all_medias[id] = {"type": o.type, "canvas": url, "parent": ci.parent_cv, "self": murl, "orgp": o.orgp};
			if(o.type === "Canvas") that.all_medias[id].cvinst = new Subcanvas(murl, id, ci, Mia.osdv);
			o.done = true;
		}
		function add_durs(durs, st, ed, id, type, bst){
			if(!durs[st]) durs[st] = {};
			Mut.arr.add(durs[st], ed, {"id": id, "label": Mut.uri.filename(id), "type": type, "bst": bst});;
		}
		function time_mode(id, o, st, bdur, cvdur, bst, urepo){
			...
			case "loop":
				...
				for(var t=st+bdur,max=st+cvdur; t<max; t+=bdur){
					Mut.arr.add(urepo.trim, t, {"id": id, "bst": bst});
				}
				break;
			case "trim":
			default:
	...
	gen_id: function(base, o){
		return base + (o.type === "Audio" ? "" : 
			(o.loc ? o.loc.replace(/^#/, "@") : "") +
			((o.type === "Text" && o.class === "paint") ? "_" + Mut.str.easy_fingerprint(o.val) : "")
	...
	get: function(type){
		if(Mav.vinfo.bgtile && Mav.type === "audio" && !Mavclock.use){
	...
	showhide_item: function(m, op, now){
		var minfo = Mavannot.all_medias[m.id];
		if(minfo.type === "Image"){
	...
	show: {
		avc: function (m, now, minfo){
			...
			if(elt.tagName.toLowerCase() === "div"){
				if(now === 0 && !Mavclock.playing && elt.className === "texthighlight") return;
				elt.style.display = "block";
				Mavannot.up_media[m.id] = [now, m.end, m.bst];
				return;
			}else elt.style.display = "block";
			if(! Mavclock.seeked){
				if(now !== m.bst){
					try{
						elt.currentTime = m.bst;
					}catch(e){
						console.error(e.message, "when seeking from", elt.currentTime, "to", m.bst);
					}
				}else if(now === 0){
					elt.currentTime = 0;
		...
		img: function(m, now, minfo){
			var itemidx = Mavannot.img_layer.check(m.id, minfo);
			if(itemidx === -1) return;
			Mavannot.up_media[m.id] = [now, m.end, null];
			if(Mavannot.img_layer.viewer.world.getItemCount()===0){
				var c=0, sid = setInterval(function(){
					if(Mavannot.img_layer.viewer.world.getItemCount() || c++ > 10){
						Mavannot.img_layer.setopacity(1, itemidx);
						clearInterval(sid);
					}else if(c++ > 10) clearInterval(sid);
				}, 200);
			}else Mavannot.img_layer.setopacity(1, itemidx);
		}
	},
	
	mute: {
		current: function(to_hide){
			Object.keys(Mavannot.up_media).forEach(function(id){
				var itemidx = Mavannot.img_layer.check(id);
				if(itemidx === -1) {
					if(Mav.melts[id]) this.avc(id, to_hide);
					else delete Mavannot.up_media[id];
		...
		avc: function(id, to_hide){
			if(to_hide){
				Mav.melts[id].style.display = "none";
				delete Mavannot.up_media[id];
			}
			if(Mavclock.playing && Mav.melts[id].tagName.toLowerCase() !== "div") Mav.melts[id].pause();
		},
		img: function(id, minfo, to_hide){
			var itemidx = Mavannot.img_layer.check(id, minfo);
			if(itemidx === -1) return;
			if(to_hide) delete Mavannot.up_media[id];
			Mavannot.img_layer.setopacity(0, itemidx);
		}
	},
	img_layer: {
		items: null,
		...
		get_layer: function(id, mi){
			if(!mi) mi = Mavannot.all_medias[id];
		...
		is_image: function(id){
			return Mavannot.all_medias[id].type === "Image";
		}
	},
	...
	clear: function(an){
		...
		if(!this.pre_expose){
			...
			annotorious.removeAnnotation(an);
...
var Mavclock = {
	...
	setup: function(){
		...
		this.bar.addEventListener("change", function(ev) {
			Mavclock.update(Math.floor(ev.target.value));
		});
	...
	add_timeline: function(Dur){
		...
		Object.keys(Mavannot.durs).sort(Mut.num.nasc).forEach(function(st){
			Object.keys(Mavannot.durs[st]).sort(Mut.num.nasc).forEach(function(ed){
				Mavannot.durs[st][ed].forEach(function(obj){
					...
					sp.onclick = function(ev){Mavclock.jump(ev, Number(st), Number(ed));};
					sp.style.cursor = "pointer";
					div.appendChild(sp);
					Mavclock.tl.appendChild(div);
					count++;
				});
			});
		});
	...
	update: function(now){
		var bartime;
		if(now !== undefined){
			Mavclock.current = now * 1000;
			bartime = now;
		}else{
			Mavclock.current += Mavclock.delay;
			bartime = Mavclock.current / 1000;
			now = Math.floor(bartime);
		}
		Mavclock.now = now;
		if(Mavclock.current > Mavclock.end){
			window.clearInterval(Mavclock.timer);
			Mavannot.mute.current(false);
			if(Miiif.hint.auto_advance){
				Mav.turn_page(Mav.key_uri(), 1);
			}else{
				Mavclock.btn.innerText = "replay";
			}
			if(!Mav.check_autoturn()) Mavclock.btn.innerText = "replay";
		}else{
			Mavclock.bar.value = bartime;
			if(now !== Mav.state.lastsec){
				var lastsec = Mav.state.lastsec;
				Mavannot.check(now, Mav.state.lastsec);
				Mavclock.set_disp(now);
				if(now < lastsec) Mavclock.test_seek(now, 0, lastsec);
				else if(now - lastsec > 1) Mavclock.test_seek(now, Mavclock.last_st - 1, now);
			}
			Mav.state.lastsec = now;
		}
	...
	pause: function(){
		...
		for(var id in Mavannot.up_media){
			var elt = Mav.melts[id];
			if(Mavannot.img_layer.check(id) === -1 && elt.pause) elt.pause();
		}
	...
	resume: function(){
		...
		for(var id in Mavannot.up_media){
			var elt = Mav.melts[id];
			if(Mavannot.img_layer.check(id) === -1 && elt.play) elt.play();
		}
		this.btn.innerText = "pause";
	},
	btn_action: function(e){
		...
		case "replay":
			...
			Mavannot.clear_current();
			Mavannot.mute.current();
		case "start":
			Mavclock.start();
			break;
		case "pause":
	...
	test_seek: function(now, from, to){
		...
		for(var st in Mavannot.durs){
			if(st < from) continue;
			if(st > to) break;
			this.last_st = Number(st);
			for(var ed in Mavannot.durs[st]){
				hide = (now < st || now > ed);
				Mavannot.durs[st][ed].forEach(function(m){
					if(hide && dont_hide.indexOf(m.id)===-1){
						if(m.type === "annot"){
							Mavannot.clear(m.anno);
							if(!m.anno.has_region) Mavannot.test_overlap(now, 1, m.anno);
						}else if(Mavannot.up_media[m.id]){
							Mavannot.showhide_item(m, 0);
							if(now < st) this.seek_media(m.id, now, st, m.bst);
						}
					}else if(!hide){
						if(m.type === "annot"){
							Mavannot.expose(m.anno, now);
						}else{
							this.seek_media(m.id, now, st, m.bst);
							if(! Mavannot.up_media[m.id]) Mavannot.showhide_item(m, 1, now);
						}
						dont_hide.push(m.id);
					}
				}, this);
	...
	seek_media: function(id, now, st, bst){
		var is_image;
		if((is_image = Mavannot.img_layer.is_image(id))) return;
		if(!st) st = Mavannot.up_media[id][0];
		if(!is_image) Mav.melts[id].currentTime = now - st + (bst || 0);
...
Subcanvas.prototype = {
	...
	osd_setup: function(o){
		...
		if(!o.pos){
			...
			o.pos = [0, 0, this.posdv.elt.clientWidth / 2, this.posdv.elt.clientHeight / 2];
		...
		dim = new OpenSeadragon.Point(o.pos[2], o.pos[3]);
		viewport.containerSize = viewport._containerInnerSize = dim;
		this.original_pos = o.pos;
		this.current_pos = Mut.obj.copy(o.pos);
...
var Mau = {
	...
	init: function(aurl){
		...
		Mavannot.noreg.pos = this.annopos;
...
var Mytb = {
	...
	init: function(vurl){
		...
		this.vid = uparam[2];
		Mia.elt.media = Mav.v = Mut.dom.elt("div", "", [["id", "player"]]);
		Mav.setup_elt(); 
	...
	onPlayerReady: function(ev) {
		Mia.elt.media = Mav.v = Mut.dom.get(Mav.v.id, "id");
		...
		Mavannot.set_rewind(Mytb.sec_now(), true);
	...
	onPlayerStateChange: function(ev) {
		if(Mav.state.keeper === Mytb.c.SEEK_WAIT){
			if(ev.data === YT.PlayerState.PLAYING){
				...
				Mavannot.check(Mav.state.lastsec, Mav.state.lastsec-1);
			}
	...
	watch_state: function(ev){
		...
		function time_keeper(){
			if((now = Mytb.sec_now()) > Mav.state.lastsec){
				...
				Mavannot.set_rewind(now);
				Mavannot.check(now, now-1);
				Mav.state.lastsec = now;
			}
		}
	},
